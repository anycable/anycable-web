...
const billingRadios = document.querySelectorAll<HTMLInputElement>('input[name="billing"]')
const priceAmounts = document.querySelectorAll<HTMLElement>('.pricing-card__amount')
...
// Pricing Calculator
const connectionsSlider = document.getElementById('connections-slider') as HTMLInputElement
...
function updateCalculator() {
  if (!connectionsSlider || !connectionsValue || !anycablePrice || !pusherPrice || !selfHostedPrice || !pusherSavings) {
    return
  }
  ...
  if (pricing.anycable.cost === 0) {
    ...
    if (recommendedCard) {
      const title = recommendedCard.querySelector('.pricing-result-card__title')
      if (title) title.textContent = 'AnyCable ' + pricing.anycable.tier
      const features = recommendedCard.querySelector('.pricing-result-card__features')
      if (features) features.textContent = `• Up to ${formatNumber(connections)} connections\n• Free forever\n• Community support`
    }
  ...
  if (pricing.anycable.cost === 0 || pricing.pusher === 0) {
    ...
    const savings = Math.round(((pricing.pusher - pricing.anycable.cost) / pricing.pusher) * 100)
    if (savings > 0) {
      pusherSavings.textContent = `Save ${savings}%`
      pusherSavings.style.color = '#4caf50'
    } else {
      const extraCost = Math.round((Math.abs(savings) / 100) * pricing.anycable.cost)
...
async function loadGitHubStars() {
  try {
    const response = await fetch('https://api.github.com/repos/anycable/anycable')
